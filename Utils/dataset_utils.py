...
def from_nested_to_long_class(X, y):
    '''
    ...
    for serie_id, serie_class in zip_longest(pd.unique(X['id']), y):
        new_y.extend([serie_class for _ in range(
            X[X['id'] == serie_id].shape[0])])
...
def import_heartbeat_dataset(heartbeat_path: str):
    ...
    y_train = from_nested_to_long_class(train, y_train)
    y_test = from_nested_to_long_class(test, y_test)
...
def import_epilepsy_dataset(epilepsy_path: str):
    ...
    y_train = from_nested_to_long_class(train_df, y_train)
    y_test = from_nested_to_long_class(test_df, y_test)
